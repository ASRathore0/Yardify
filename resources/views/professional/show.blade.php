...
@endphp
    ...
    <!-- Hero Section (Slider) -->
    <div class="relative w-full h-[40vh] sm:h-[40vh] mt-[px] group">
        <!-- Slider Images -->
        ...
        <!-- Gradient Overlay (Softened for image visibility) -->
        <div class="absolute inset-0 bg-gradient-to-t from-slate-900/40 to-transparent pointer-events-none"></div>
    </div>

    <!-- Profile Header Section (Below Hero) -->
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6 relative z-20">
        <div class="flex flex-col sm:flex-row gap-3 sm:gap-6 items-start sm:items-end">
            <!-- Profile Avatar (Overlapping Hero) -->
            <div class="w-24 h-24 sm:w-32 sm:h-32 bg-white rounded-2xl p-1.5 shadow-md shrink-0 overflow-hidden relative border border-slate-100 -mt-10 sm:-mt-16 bg-slate-50">
                <img src="{{ $vendor->user->avatar_url ?? asset('imagee/profile.png') }}" class="w-full h-full object-cover rounded-xl" alt="Profile">
                <div class="absolute top-2.5 right-2.5 bg-green-500 w-3.5 h-3.5 rounded-full border-2 border-white shadow-sm" title="Online"></div>
            </div>
            ...
            <div class="flex-1 pb-1 w-full">
                ...
                <div class="flex items-center justify-between">
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">{{ $vendor->service_name ?? $vendor->title ?? 'Professional Service' }}</h1>
                    <!-- Mobile Verified Badge -->
                    <span class="sm:hidden bg-blue-100 text-blue-700 text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wide border border-blue-200">Verified</span>
                </div>
                
                <div class="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-5 mt-1 sm:mt-2">
                    <!-- Vendor Personal Name + Verified Social Icon -->
                    <div class="flex items-center gap-1.5 text-slate-700 font-bold text-lg">
                        {{ $vendor->user->name ?? 'Vendor Profile' }}
                        <i class="fas fa-certificate text-blue-500 text-sm mt-0.5" title="Verified Professional"></i>
                    </div>
            ...
            <!-- Desktop Right Side Controls / Badge -->
            <div class="hidden sm:flex flex-col items-end pb-1 shrink-0">
                <span class="bg-blue-100 text-blue-700 text-xs font-black px-3 py-1.5 rounded-lg uppercase tracking-wider border border-blue-200 flex items-center gap-1.5 shadow-sm">
    ...
    <!-- Main Content -->
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-2 sm:pt-4 pb-0 lg:pb-12">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <!-- Services Card -->
                <div class="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-100">
                    <h2 class="text-lg sm:text-xl font-bold text-slate-800 mb-0 flex items-center gap-2">
            ...
            <div class="lg:col-span-1">
                ...
                <div class="fixed bottom-20 left-4 right-4 z-40 lg:hidden">
                    @auth
                    <button onclick="toggleMobileBooking()" class="w-full bg-[#046c9f] hover:bg-[#035b88] text-white px-8 py-4 rounded-xl font-bold shadow-2xl shadow-[#046c9f]/40 transition-all flex justify-center items-center gap-2 active:scale-95 text-lg">
                        Book Now <i class="fas fa-arrow-right"></i>
                    </button>
                    @else
                    <a href="{{ route('login') }}" class="w-full bg-[#046c9f] hover:bg-[#035b88] text-white px-8 py-4 rounded-xl font-bold shadow-2xl shadow-[#046c9f]/40 transition-all flex justify-center items-center gap-2 active:scale-95 text-lg">
                        Login to Book <i class="fas fa-sign-in-alt"></i>
                    </a>
                    @endauth
                </div>

                <!-- Sticky container for Desktop / Bottom Sheet for Mobile -->
                <div id="bookingContainer" class="fixed inset-0 z-[100] bg-slate-900/60 hidden opacity-0 transition-opacity duration-300 lg:block lg:static lg:bg-transparent lg:opacity-100 lg:z-auto">
                                    ...
                                    <form id="bookingForm" onsubmit="event.preventDefault(); submitBooking();">
                                        ...
                                        <div class="mt-6 pt-5 border-t border-slate-200 border-dashed">
                                            ...
                                            <div class="flex justify-between items-center pt-4 border-t border-slate-200">
                                                ...
                                                <span id="summaryTotal" class="text-2xl font-black text-[#046c9f]">₹{{ ($vendor->services->first()->price ?? $vendor->base_price ?? 500) }}</span>
                                            </div>
                                        </div>

                                        <div class="mt-8 pb-14 lg:pb-0">
                                            @auth
                                            <button id="submitBtn" type="submit" class="w-full bg-[#046c9f] hover:bg-[#046c9f] text-white font-bold py-4 rounded-xl shadow-xl shadow-blue-600/30 transition-all flex justify-center items-center gap-2 text-lg outline-none focus:ring-4 focus:ring-blue-500/30 active:scale-[0.98]">
                                                Confirm Details <i class="fas fa-arrow-right text-sm"></i>
                                            </button>
                                            @else
                                            <a href="{{ route('login') }}" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-xl shadow-blue-600/30 transition-all flex justify-center items-center gap-2 text-lg outline-none focus:ring-4 focus:ring-blue-500/30 active:scale-[0.98]">
                                                Login to Book <i class="fas fa-sign-in-alt text-sm"></i>
                                            </a>
                                            @endauth
    ...
    <script>
        ...
        function selectService(element, name, price) {
            ...
            const total = parseFloat(price); // Platform fee is 0
